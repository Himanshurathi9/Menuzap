...
// Mock data for the customer view
const cafeData = {
...
export default function PublicMenuPage({ params }: { params: { cafeSlug: string } }) {
  // In a real app, we would fetch cafe details based on params.cafeSlug here

  return (
    <main className="min-h-screen bg-background pb-24 relative selection:bg-amethyst selection:text-white">
      {/* Dynamic Background Glow */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[80%] h-[300px] bg-amethyst/10 blur-[120px] rounded-full pointer-events-none z-0"></div>
          ...
          <div key={item.id} className="glass-panel rounded-[var(--radius)] overflow-hidden flex flex-col group">
            ...
            <div className="h-48 w-full bg-card/50 flex items-center justify-center text-7xl relative overflow-hidden">
              ...
              <div className="absolute top-4 right-4 bg-background/80 backdrop-blur-sm p-1.5 rounded border border-white/10">
                 <div className={`w-3 h-3 rounded-sm border-2 flex items-center justify-center ${item.isVeg ? 'border-success' : 'border-danger'}`}>
                    <div className={`w-1 h-1 rounded-full ${item.isVeg ? 'bg-success' : 'bg-danger'}`}></div>
                 </div>
              </div>
            </div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm text-vapor-muted leading-relaxed line-clamp-2">
                  {item.description}
                </p>
              </div>
              
              <button className="mt-5 w-full py-3 rounded-lg font-bold border border-white/10 bg-white/5 text-vapor hover:bg-amethyst hover:text-white hover:border-amethyst transition-all shadow-sm hover:shadow-glow-amethyst">
                Add to Order
              </button>
            </div>
          </div>
      ...
      {/* Floating Cart Button (Pops up when items are added) */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-11/12 max-w-md z-50">
        <div className="bg-vapor text-background px-6 py-4 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.5)] flex justify-between items-center cursor-pointer hover:scale-[1.02] transition-transform">
          <div className="flex items-center gap-3">
            <div className="bg-background text-vapor w-8 h-8 rounded-full flex items-center justify-center font-bold">2</div>
            <div>
              <p className="font-bold text-sm">View your order</p>
              <p className="text-xs font-semibold opacity-70">Table 4</p>
            </div>
          </div>
